...
export const TransferDialog = ({ open, onOpenChange }: TransferDialogProps) => {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    if (!user || !fromAccountId || !toAccountId || !amount) {
      toast({ variant: "destructive", title: "Campos obrigatórios", description: "Preencha todos os campos para realizar a transferência." });
      return;
    }

    const transferAmount = parseFloat(amount);
    if (transferAmount <= 0) {
      toast({ variant: "destructive", title: "Valor inválido", description: "O valor da transferência deve ser maior que zero." });
      return;
    }

    if (fromAccount && transferAmount > fromAccount.balance) {
      toast({ variant: "destructive", title: "Saldo insuficiente", description: `A conta de origem tem apenas ${formatCurrency(fromAccount.balance)} disponível.` });
      return;
    }
    ...
    try {
      ...
      const { error: txError } = await supabase.from("transactions").insert([
        {
          ...
          category: "Transferência", type: "expense", amount: transferAmount,
          status: "pagamento_concluido", date: today, paid_date: today,
          payment_method: "bank_transfer", bank_account_id: fromAccountId, tag: "transferencia",
        },
        {
          user_id: user.id, household_id: householdId,
          description: `${transferDescription} ← ${fromAccount?.bank_name}`,
          category: "Transferência", type: "income", amount: transferAmount,
          status: "pagamento_concluido", date: today, paid_date: today,
          payment_method: "bank_transfer", bank_account_id: toAccountId, tag: "transferencia",
        },
      ]);
      ...
      invalidateAccounts();
      toast({ title: "Transferência realizada", description: `${formatCurrency(transferAmount)} transferido de ${fromAccount?.bank_name} para ${toAccount?.bank_name}.` });
            ...
            {accounts.map((account) => (
              <SelectItem key={account.id} value={account.id}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">{account.bank_name} - {account.name}</span>
                  <span className="text-xs text-muted-foreground ml-auto">{formatCurrency(account.balance)}</span>
                </div>
              </SelectItem>
            ))}
          </SelectContent>
