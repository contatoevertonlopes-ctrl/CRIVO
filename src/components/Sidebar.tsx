  ...
  return <>
      ...
      <aside className={cn("fixed lg:relative z-50 flex flex-col gap-6 bg-sidebar border-r border-sidebar-border px-5 py-6 min-h-screen transition-all duration-300 ease-in-out", collapsed ? "w-[72px] px-3" : "w-64", "lg:translate-x-0", collapsed ? "-translate-x-full lg:translate-x-0" : "translate-x-0")}>
        ...
        <nav className="flex flex-col gap-6">
          <div>
            {!collapsed && <div className="text-[11px] uppercase tracking-[0.16em] text-muted-foreground mb-2 mt-3">
                Navegação
              </div>}
            <ul className="flex flex-col gap-1">
              ...
              return <li key={item.path} onClick={() => navigate(item.path)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200", collapsed && "justify-center px-2", isActive ? "bg-gradient-to-r from-primary/10 to-blue-500/10 border border-primary/50 text-foreground" : "text-muted-foreground hover:bg-secondary/90 hover:border-border/40 hover:text-foreground border border-transparent")} title={collapsed ? item.label : undefined}>
              ...
              {isAdmin && <li onClick={() => navigate("/admin")} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200", collapsed && "justify-center px-2", location.pathname === "/admin" ? "bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/50 text-foreground" : "text-yellow-400 hover:bg-yellow-500/10 hover:border-yellow-500/40 hover:text-yellow-300 border border-transparent")} title={collapsed ? "Administração" : undefined}>
        ...
        <div className="mt-auto pt-3 border-t border-sidebar-border flex flex-col gap-3">
          {user ? subscribed ? <>
                <div className={cn("inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary/40 bg-primary/10 text-[11px] text-green-200 cursor-pointer hover:bg-primary/20 transition-colors", collapsed && "justify-center px-2")} onClick={() => navigate("/plans")} title={collapsed ? "Plano Pro Ativo" : undefined}>
                  <Crown className="w-4 h-4 shrink-0" />
                  {!collapsed && <span>Plano Pro Ativo</span>}
                </div>
                {!collapsed && <p className="text-xs text-muted-foreground leading-relaxed">
                    Visualize fluxo de caixa, recorrências e previsões em tempo real.
                  </p>}
              </> : (/* Pro Offer for non-subscribers */
        <div onClick={() => navigate("/plans")} className={cn("relative overflow-hidden rounded-xl cursor-pointer transition-all hover:scale-[1.02]", collapsed ? "p-2 bg-gradient-to-b from-primary/20 to-primary/5 border border-primary/40" : "p-3 bg-gradient-to-b from-primary/15 to-transparent border border-primary/40 shadow-[0_0_30px_rgba(34,197,94,0.2)]")} title={collapsed ? "Assinar Pro" : undefined}>
              ...
              </div>) : <button onClick={() => navigate("/auth")} className={cn("text-sm text-primary hover:text-primary/80 transition-colors text-left", collapsed && "text-center")}>
      ...
      <div className={cn("hidden lg:block shrink-0 transition-all duration-300", collapsed ? "w-[72px]" : "w-64")} />
    </>;
};
