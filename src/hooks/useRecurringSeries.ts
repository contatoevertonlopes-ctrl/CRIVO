...
/**
 * Creates a recurring_series record and a SINGLE first transaction.
 * Subsequent occurrences are generated automatically by DB triggers:
 * - On payment (trigger: process_recurrence_on_payment)
 * - On due date (cron → process_recurrence_on_due_date)
 */
export const createRecurringSeries = async (
  ...
  // 2. Insert only the first occurrence
  const { error: txError } = await supabase.from("transactions").insert({
    ...
    paid_date: firstOccurrence.paid_date,
    tag: data.tag,
    payment_method: data.payment_method,
    is_recurring: true,
...
/**
 * Deletes all remaining (unpaid) occurrences of a series and the series record itself.
 */
